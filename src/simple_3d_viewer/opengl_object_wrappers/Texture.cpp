...
uint8_t *Texture::loadImageAtPath(int pathsIndex, bool flip) {
  ...
  uint8_t *loadedImage =
      stbi_load((paths_[pathsIndex]).c_str(), &widths_[pathsIndex],
                &heights_[pathsIndex], &colorChannelsCounts_[pathsIndex], 0);
  if (!loadedImage) {
...
void Texture::createTexture2D() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, format, widths_.front(), heights_.front(), 0,
               format, GL_UNSIGNED_BYTE, loadedImages_.back());
...
void Texture::createTextureCubeMap() {
  ...
  for (int i = 0; i < loadedImages_.size(); ++i) {
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, widths_[i],
                 heights_[i], 0, GL_RGB, GL_UNSIGNED_BYTE, loadedImages_[i]);
