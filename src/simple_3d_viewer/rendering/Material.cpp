...
static inline void setSamplerUniform(Program &program,
...
static inline void setUVChannelUniform(Program &program,
                                       const std::string &uniformNameBase,
...
static void assignSlotsToTextures(std::vector<Material::TextureData> &textures,
                                  const std::string &texturesType,
                                  Program &program, uint32_t &slotToAssign) {
  ...
  for (uint32_t i = 0; i < texturesCount; ++i) {
    ...
    setUVChannelUniform(program, texturesType + "UVChannel", i,
                        textureData.uvChannel);
  }
}
...
void Material::prepareTexturesForUse(Program &program) {
  ...
  assignSlotsToTextures(diffuseRoughnessTextures_, "diffuseRoughness", program,
                        slotToAssign);
}

void Material::setUniforms(Program &program) {
  program.doOperations([this](Program &program) {
    ...
    program.setInt("diffuseTexturesCount",
                   static_cast<int>(diffuseTextures_.size()));
    program.setInt("specularTexturesCount",
                   static_cast<int>(specularTextures_.size()));
    program.setInt("emissiveTexturesCount",
                   static_cast<int>(emissiveTextures_.size()));
    program.setInt("normalsTexturesCount",
                   static_cast<int>(normalsTextures_.size()));
    program.setInt("metalnessTexturesCount",
                   static_cast<int>(metalnessTextures_.size()));
    program.setInt("diffuseRoughnessTexturesCount",
