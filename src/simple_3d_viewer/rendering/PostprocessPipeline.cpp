...
namespace Simple3D
{
      ...
      const std::vector<PostprocessID> &postprocessIDs)
  {
    ...
    {
      auto lowerCaseId = id;
      std::ranges::transform(lowerCaseId, begin(lowerCaseId), [](unsigned char c) -> char { return std::tolower(c); });
      idToPostprocess.emplace(std::pair{ id, Postprocess{ Program(vertexShaderFilename, lowerCaseId), false } });
    }

    return idToPostprocess;
  }

  PostprocessPipeline::PostprocessPipeline(const std::vector<std::string> &postprocessIDs, Size size)
  ...
  void PostprocessPipeline::initPostprocesses()
  {
    for (auto &[id, postprocess] : idToPostprocess_)
    {
      ...
      postprocess.program.doOperations([](Program &program) { program.setInt("screenTexture", screenTextureSlot); });
    }
  }
  ...
  void PostprocessPipeline::makeFramebuffers()
  {
    ...
    for (int i = 0; i < framebuffers_.size(); ++i)
    {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size_.width, size_.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorBuffers_[i], 0);
      ...
      glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, renderBuffers_[i]);
  ...
  {
    static constexpr std::array screenQuadVertices = { -1.0f, 1.0f,  0.0f, 1.0f, -1.0f, -1.0f, 0.0f, 0.0f,
  ...
  void PostprocessPipeline::finalize()
  {
    ...
    for (int i = 0; i < postprocessesOrder_.size() - 1; ++i)
    {
      ...
      glBindTexture(GL_TEXTURE_2D, colorBuffers_[i]);
      postprocessesOrder_[i]->doOperations([](Program &program) { glDrawArrays(GL_TRIANGLES, 0, 6); });
      ++colorBufferToUse;
    }
    ...
    postprocessesOrder_.back()->doOperations([](Program &program) { glDrawArrays(GL_TRIANGLES, 0, 6); });
  ...
  void PostprocessPipeline::updatePostprocesses()
  {
    for (Program *program : postprocessesToUpdate_)
      program->doOperations(
          ...
          { program.setVec3f("inverseScreenSize", glm::vec3(1.0f / size.width, 1.0f / size.height, 1.0f)); });
  }
