...
namespace Simple3D {
class Texture {
  enum class Type { Texture2D, TextureCubeMap };
...
public:
  ...
  Texture(const std::vector<std::string> &paths, bool doOpenGLStuff = true)
      : paths_(paths), type_(Type::TextureCubeMap) {
  ...
  Texture(Texture &&texture) noexcept
      : texture_(texture.texture_), paths_(std::move(texture.paths_)),
        ...
        loadedImages_(std::move(texture.loadedImages_)), type_(texture.type_) {
    texture.texture_ = 0;
  }
  ...
  const std::string &getPath() const { return paths_.front(); }
  const std::vector<std::string> &getPaths() const { return paths_; }
  int getWidth() const { return widths_.front(); }
  int getHeight() const { return heights_.front(); }
